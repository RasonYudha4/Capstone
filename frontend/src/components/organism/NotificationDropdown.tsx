...
import { documentKeys } from '@/hooks/useDocument'
...
export default function NotificationDropdown({ onViewAll }: NotificationDropdownProps) {
    ...
    useNotificationEvents(
        useCallback((event) => {
            if (event.type === 'sync') {
                console.log('[SSE]', event.type, 'unread count:', event.count)
            } else {
    ...
    async function openReviewModal(item: NotificationItem) {
        const document = toDocumentResponse(item)
        if (!document) {
    ...
    function handleNotificationClick(item: NotificationItem) {
        if (!item.Read) handleMarkRead(item.NotificationID)
        void openReviewModal(item)
    }
